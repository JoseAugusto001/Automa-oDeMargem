...
import config
from robo.passivos.cpf_utils import cpf_com_mascara, cpf_valido_11
from robo.passivos.csv_io import log_critico, salvar_dataframe_final
from robo.comms.fluxo_consulta import garantir_cpf_preenchido, historico_tem_linha_sucesso_cpf, pagina_tem_cpf_invalido, pagina_tem_cpf_nao_encontrado, pagina_tem_erro_na_consulta, pagina_tem_registro_nao_encontrado, pagina_tem_restricao_emissao, selecionar_banco
...
def processar_clientes(page: Page, clientes: Iterable[Cliente], caminho_saida: str) -> None:
    ...
    for idx, cliente in enumerate(clientes):
        ...
        if not cpf_valido_11(cpf_raw):
            log_critico(lista_saida, cliente, "", "cpf_invalido", "CPF com tamanho diferente de 11 dígitos (provável perda no CSV)")
            pular = True
        ...
        try:
            ...
            try:
                ...
                pass
            if pagina_tem_restricao_emissao(page):
                try:
                ...
                log_critico(lista_saida, cliente, "", "restricao_emissao", msg_restricao.replace("\n", " ").replace("\r", ""))
                voltar_para_consulta_limpa(page)
                continue
            campo_cpf = (
            ...
            for banco_atual in ["QiTech", "Celcoin"]:
                ...
                if not selecionar_banco(page, banco_atual):
                    log_critico(lista_saida, cliente, banco_atual, "erro_selecao_banco", "Não foi possível selecionar o banco no formulário")
                    pular_cliente = True
                    break
                try:
                ...
                page.wait_for_timeout(150)
                garantir_cpf_preenchido(page, cpf_site)
                page.wait_for_timeout(150)
                try:
                    ...
                    pass
                if processar_resultado_existente_no_historico(page, cpf_site, banco_atual, cliente, lista_saida, timeout_ms):
                    continue
                garantir_cpf_preenchido(page, cpf_site)
                ...
                try:
                    ...
                    def resultado_apareceu() -> bool:
                        p = pg_consulta
                        if pagina_tem_restricao_emissao(p): return True
                        if pagina_tem_cpf_invalido(p): return True
                        if pagina_tem_cpf_nao_encontrado(p): return True
                        if pagina_tem_registro_nao_encontrado(p): return True
                        if pagina_tem_erro_na_consulta(p): return True
                        try:
                    ...
                    for _ in range(max(1, pausa_celcoin // 300)):
                        try:
                            if any(page.get_by_text(t, exact=False).first.is_visible() for t in textos_modal):
                                break
                            if extrair_link_termo_pagina(page):
                                break
                        except Exception:
                            pass
                ...
                if pagina_tem_restricao_emissao(page):
                    try:
                        ...
                        msg_restricao = config.UI_TEXTO_RESTRICAO_EMISSAO
                    log_critico(lista_saida, cliente, banco_atual, "restricao_emissao", msg_restricao.replace("\n", " ").replace("\r", ""))
                    voltar_para_consulta_limpa(page)
                    continue
                page.wait_for_timeout(200)
                if pagina_tem_cpf_invalido(page) and not historico_tem_linha_sucesso_cpf(page, cpf_site):
                    try:
                    ...
                    log_critico(lista_saida, cliente, banco_atual, "cpf_invalido", msg_cpf)
                    pular_cliente = True
                    voltar_para_consulta_limpa(page)
                    break
                if pagina_tem_cpf_nao_encontrado(page):
                    try:
                        loc_nao_enc = page.get_by_text(config.UI_TEXTO_CPF_NAO_ENCONTRADO, exact=False).or_(page.get_by_text(getattr(config, "UI_TEXTO_CPF_NAO_ENCONTRADO_ALT", ""), exact=False)).first
                        msg_nao_enc = loc_nao_enc.inner_text()[:300].replace("\n", " ").replace("\r", "") if loc_nao_enc.is_visible() else config.UI_TEXTO_CPF_NAO_ENCONTRADO
                    except Exception:
                        msg_nao_enc = config.UI_TEXTO_CPF_NAO_ENCONTRADO
                    log_critico(lista_saida, cliente, banco_atual, "cpf_nao_encontrado", msg_nao_enc)
                    voltar_para_consulta_limpa(page)
                    break
                url_termo = None
                ...
                if modal_visivel:
                    ...
                    if not url_termo and banco_atual and "celcoin" in banco_atual.lower():
                        url_termo = extrair_link_termo_pagina(page)
                    if not url_termo:
                        status = "falha_modal_autorizacao"
                        mensagem_erro = "Não consegui extrair a URL do termo."
                        log_critico(lista_saida, cliente, banco_atual, "falha_modal_autorizacao", mensagem_erro)
                        voltar_para_consulta_limpa(page)
                        continue
                if not modal_visivel and not url_termo and banco_atual and "celcoin" in banco_atual.lower():
                    url_termo = extrair_link_termo_pagina(page)
                if not modal_visivel and not url_termo and pagina_tem_erro_na_consulta(page):
                    ...
                    try:
                        ...
                        try:
                            ...
                            pass
                    log_critico(lista_saida, cliente, banco_atual, "erro_na_consulta", msg_erro)
                    voltar_para_consulta_limpa(page)
                    continue
                ...
                if url_termo:
                    ...
                    try:
                        ...
                        origin_aba = f"{parsed_aba.scheme}://{parsed_aba.netloc}"
                        page.context.grant_permissions(["geolocation"], origin=origin_aba)
                    except Exception:
                        pass
                    passo_termo = "inicio"
                    try:
                        ...
                        if not req_mal_visivel and texto_req_mal_alt:
                            try:
                                ...
                                pass
                        if req_mal_visivel:
                            fechar_pagina_se_aberta(aba_termo)
                            page.bring_to_front()
                            voltar_para_consulta_limpa(page)
                            raise TermoRequisicaoMalFormatada()
                        texto_proc = getattr(config, "UI_TEXTO_TERMO_EM_PROCESSAMENTO", "em processamento")
                        try:
                            banner_processamento_visivel = aba_termo.get_by_text(texto_proc, exact=False).first.is_visible()
                        except Exception:
                            banner_processamento_visivel = False
                        if banner_processamento_visivel:
                            ...
                            if not processamento_resolvido:
                                log_critico(lista_saida, cliente, banco_atual, "termo_em_processamento", "Termo em processamento; aguardado tempo máximo")
                                fechar_pagina_se_aberta(aba_termo)
                                page.bring_to_front()
                                voltar_para_consulta_limpa(page)
                                continue
                        ...
                        try:
                            ...
                            log_critico(lista_saida, cliente, banco_atual, "falha_termo_autorizacao", "Botão ENVIAR não apareceu a tempo")
                            fechar_pagina_se_aberta(aba_termo)
                            page.bring_to_front()
                            voltar_para_consulta_limpa(page)
                            continue
                        ...
                        if not obrigado_ok:
                            try:
                                if aba_termo.get_by_text(texto_proc_apos, exact=False).first.is_visible():
                                    log_critico(lista_saida, cliente, banco_atual, "termo_em_processamento", "Termo em processamento após envio; aguardado tempo máximo")
                                    fechar_pagina_se_aberta(aba_termo)
                                    page.bring_to_front()
                                    voltar_para_consulta_limpa(page)
                                    continue
                        ...
                        if termo_processamento_visivel:
                            ...
                            log_critico(lista_saida, cliente, banco_atual, "falha_termo_autorizacao", "Confirmação do termo não obtida no tempo esperado")
                        else:
                            log_critico(lista_saida, cliente, banco_atual, "falha_termo_autorizacao", f"Termo ({passo_termo}): {tipo_termo}: {msg_termo}")
                        fechar_pagina_se_aberta(aba_termo)
                        page.bring_to_front()
                        voltar_para_consulta_limpa(page)
                        continue
                ...
                if vinculo_visivel:
                    ...
                    log_critico(lista_saida, cliente, banco_atual, "sem_vinculo", msg_sem_vinculo)
                    voltar_para_consulta_limpa(page)
                    continue
                if status == "falha_modal_autorizacao":
                    log_critico(lista_saida, cliente, banco_atual, status, mensagem_erro)
                    voltar_para_consulta_limpa(page)
                    continue
                ...
                try:
                    try:
                        ...
                        pass
                    page.wait_for_timeout(300)
                    pagina_consulta = obter_pagina_consulta_principal(page) or (page.context.pages[0] if page.context.pages else page)
                    try:
                    ...
                    linha_cpf, locadores_linha = buscar_linha_historico(pagina_consulta, cpf_site, banco_atual, cliente, timeout_por_tentativa, max_tentativas=2, usar_recarregar=getattr(config, "USE_RECARREGAR_HISTORICO", False))
                    ...
                    for _tentativa_hist in range(max_recarregar + 1):
                        ...
                        if st == "processando":
                            if _tentativa_hist >= max_recarregar:
                                ...
                                voltar_para_consulta_limpa(page)
                                status_historico = "processando_timeout"
                                break
                            try:
                    ...
                    if banco_atual:
                        try:
                            ...
                            if banco_atual.lower() not in (texto_linha or "").lower():
                                log_critico(lista_saida, cliente, banco_atual, "falha_historico", "Linha do histórico não corresponde ao banco atual")
                                voltar_para_consulta_limpa(page)
                                continue
                        except Exception:
                            pass
                    ...
                    if linha_cpf.get_by_text(config.UI_TEXTO_SUCESSO, exact=False).first.is_visible():
                        try:
                            pagina_resultado, ok = abrir_resultado_historico(page.context, btn_ver_resultado, pagina_consulta)
                            if not ok or pagina_resultado is None:
                                raise RuntimeError("Falha ao abrir resultado")
                            pagina_resultado.bring_to_front()
                            deve_continuar, valor_extraido = tratar_recusa_ou_requisicao_mal_formatada(pagina_resultado, page, cliente, banco_atual, lista_saida)
                            if deve_continuar:
                                continue
                    ...
                    log_critico(lista_saida, cliente, banco_atual, "falha_historico", erro_msg)
                    voltar_para_consulta_limpa(page)
                    pagina_resultado = page.context.pages[0] if page.context.pages else page
                ...
                if not valor_maximo_parcela and pagina_resultado and not pagina_resultado.is_closed():
                    try:
                        v = extrair_valor_maximo_parcela(pagina_resultado)
                        if v:
                ...
                if not valor_maximo_parcela or status != "sucesso":
                    if pagina_tem_cpf_invalido(page):
                        status = "cpf_invalido"
                        try:
                            err_loc = page.get_by_text(config.UI_TEXTO_CPF_INVALIDO, exact=False).or_(page.get_by_text(getattr(config, "UI_TEXTO_CPF_INVALIDO_ALT2", "CPF informado não é válido"), exact=False)).first
                    ...
                    log_critico(lista_saida, cliente, banco_atual, status, mensagem_erro or "Erro não especificado")
                    voltar_para_consulta_limpa(page)
                    continue
                gravou_alguma_linha_simulacao = False
                try:
                    escopo_simulacao = pagina_resultado
                    try:
                        ...
                        pass
                    if escopo_simulacao == pagina_resultado and "clt/consultar" in pagina_resultado.url and linha_cpf is not None:
                        try:
                    ...
                    try:
                        ...
                        pass
                    gravou_alguma_linha_simulacao = simular_tabelas(escopo_simulacao, valor_maximo_parcela, cliente, banco_atual, lista_saida, pagina_resultado)
                except Exception:
                    try:
                        v_fallback = valor_maximo_parcela
                        if not v_fallback:
                            v_fallback = extrair_valor_maximo_parcela(pagina_resultado)
                        gravou_alguma_linha_simulacao = simular_tabelas(pagina_resultado, v_fallback, cliente, banco_atual, lista_saida, pagina_resultado)
                    except Exception:
                        pass
                if valor_maximo_parcela and not gravou_alguma_linha_simulacao:
                    ...
                    log_critico(lista_saida, cliente, banco_atual, status_sem_sim, erro_sem_sim)
                fechar_pagina_se_aberta(pagina_resultado, page)
                pagina_consulta_principal = obter_pagina_consulta_principal(page)
            ...
            try:
                ...
                log_critico(lista_saida, cliente, "", "falha_historico", erro_msg)
            except Exception:
                pass
        voltar_para_consulta_limpa(page)
    salvar_dataframe_final(caminho_saida, lista_saida)
